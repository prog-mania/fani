...
function toNumberArray(src) { // Разбиваем строку по разделителям и отбрасываем пустые подстроки.
...
function cTB(p) {
    ...
    range.addEventListener('keydown', function(event) {// запрет клавишей
...
function createUI() { // создание  trackBar, checkBox, button
...
//    id('changeImg').addEventListener('click', ()=>{changeImage(null,+1)});
//    saveImg.addEventListener('click', canvasToJpg);
    tbI.addEventListener  ('input', e => {
        imageAuto=e.target.value;
        startChangeImage()
    });
...
function resizeCanvas() { // расстановка видимых компонентов справа от canvas
    ...
    let w10=canvas.width+2, l=w10, l2=l+120+'px', left=l+'px', w=128, t=8, r=22, p=20;
...
function filesToArrays() {  // загрузка файлов в массивы
    ...
    vertices = new Float32Array(toNumberArray(files['vertices-'+image+'.txt']));
//    vertices0= vertices.slice(0);    // исходные координаты для анимации
    indices  = new Uint16Array (toNumberArray(files['indices-'+image+'.txt']));
    ...
    mB   = new Float32Array(toNumberArray(files['брови.txt']));        norm(mB);
    mV   = new Float32Array(toNumberArray(files['брови наклон.txt'])); norm(mV);

    mG   = new Float32Array(toNumberArray(files['глаза.txt']));        norm(mG);
    mW   = new Float32Array(toNumberArray(files['глаза взгляд.txt'])); norm(mW);

    mR   = new Float32Array(toNumberArray(files['рот.txt']));          norm(mR);
    mS   = new Float32Array(toNumberArray(files['рот сжатие.txt']));   norm(mS);
    mU   = new Float32Array(toNumberArray(files['улыбка.txt']));       norm(mU);
    mA   = new Float32Array(toNumberArray(files['брови выше.txt']));   norm(mA);
    mX   = new Float32Array(toNumberArray(files['наклон.txt']));       norm(mX);
}
...
var text, nText=nWav=0; // номер произносимого предложения из text и waves
...
function makeTexture(nameTex, nameImage) {
   const img = new Image();
   img.onload = function () {
      ...
      if (++loaded === 3) { // все текстуры готовы  -  рендерим
...
function addListener() { // добавка обработчиков событий
  document.addEventListener('keydown', (event)=>{ //const key = event.key;   //id('nameImg' ).innerHTML='Нажата клавиша '+key;
    switch (event.key) {
...
fani=function (p={}) { // загрузка всего и запуск анимации
   ...
   if (p.adjust!=undefined) adjust=p.adjust
...
//fani()({image:'08787.webp', back:'back6.webp', adjust:true, ani:true})
